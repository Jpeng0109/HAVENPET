...
export type ApiHealthStatus = 'checking' | 'ok' | 'disconnected' | 'unreachable' | 'waking';

function sleep(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

export async function checkApiHealth(maxAttempts = 6): Promise<ApiHealthStatus> {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      if (res.ok) return 'ok';
      // 503 = BACKEND_URL missing on Vercel proxy — config issue, not cold start
      if (res.status === 503) return 'disconnected';
      if (attempt < maxAttempts - 1) {
        await sleep(8_000);
        continue;
      }
      if (res.status === 502) return 'disconnected';
      return 'unreachable';
    } catch {
      if (attempt < maxAttempts - 1) {
        await sleep(8_000);
