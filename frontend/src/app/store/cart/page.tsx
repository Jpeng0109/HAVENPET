  ...
  async function checkout() {
    ...
    try {
      const order = await ordersApi.create(
        cart.map((c) => ({ skuId: c.skuId, quantity: c.quantity })),
      );
              ...
              <div key={item.skuId} className="flex items-center justify-between rounded-xl border bg-white p-4">
                <div>
                  ...
                  <p className="text-sm text-brand-700">
                    {item.priceLocal.toFixed(2)} {item.currency} each
                  </p>
                </div>
          ...
          {preview && (
            <div className="rounded-xl border border-slate-200 bg-white p-5 h-fit">
              ...
              <p className="text-xs text-slate-500 mt-1">
                FX rate: 1 USD = {preview.exchangeRate} {preview.currency}
              </p>
              <dl className="mt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <dt>Subtotal</dt>
                  <dd>{preview.subtotalLocal.toFixed(2)} {preview.currency}</dd>
                </div>
                <div className="flex justify-between text-slate-600">
                  <dt>VAT ({(preview.breakdown.taxRate * 100).toFixed(0)}%)</dt>
                  <dd>{preview.taxAmount.toFixed(2)}</dd>
                </div>
                <div className="flex justify-between text-slate-600">
                  <dt>Import duty ({(preview.breakdown.dutyRate * 100).toFixed(0)}%)</dt>
